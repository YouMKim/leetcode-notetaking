# 15. 3Sum

**Difficulty:** Medium  
**LeetCode Link:** [15. 3Sum](https://leetcode.com/problems/3sum/)

---

## Description

Given an integer array nums, return all the triplets `[nums[i], nums[j], nums[k]]` such that `i != j`, `i != k`, and `j != k`, and `nums[i] + nums[j] + nums[k] == 0`.

Notice that the solution set must not contain duplicate triplets.

 

Example 1:**

```

**Input:** nums = [-1,0,1,2,-1,-4]
**Output:** [[-1,-1,2],[-1,0,1]]
**Explanation:** 
nums[0] + nums[1] + nums[2] = (-1) + 0 + 1 = 0.
nums[1] + nums[2] + nums[4] = 0 + 1 + (-1) = 0.
nums[0] + nums[3] + nums[4] = (-1) + 2 + (-1) = 0.
The distinct triplets are [-1,0,1] and [-1,-1,2].
Notice that the order of the output and the order of the triplets does not matter.

```

Example 2:**

```

**Input:** nums = [0,1,1]
**Output:** []
**Explanation:** The only possible triplet does not sum up to 0.

```

Example 3:**

```

**Input:** nums = [0,0,0]
**Output:** [[0,0,0]]
**Explanation:** The only possible triplet sums up to 0.

```

 

**

---

## Constraints

- **
- `3 <= nums.length <= 3000`
- `-105 <= nums[i] <= 105`

---

## Approach

<!-- Explain your approach to solving this problem -->

---

## Solution Code

```python
class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        #given an integer and array nums #triplets that add up to 0 
        nums.sort() 
        answers = []

        for i in range(len(nums)-2):
            j = i + 1 
            k = len(nums)-1 
            if nums[i] > 0:
                break
            if i > 0 and nums[i] == nums[i-1]:
                continue 
            while j < k :
                curr_sum = nums[i] + nums[j] + nums[k]
                if curr_sum == 0 :
                    answers.append([nums[i], nums[j], nums[k]])
                    j+=1
                    k-=1
                    #SKIP dupes part 
                    while j < k and nums[j] == nums[j-1]:
                        j+=1
                    while j < k and nums[k] == nums[k+1]:
                        k-=1 
                elif curr_sum < 0:
                    j+=1
                else:
                    k-=1
                
        return answers 
                    
```

---

## Time & Space Complexity

- **Time Complexity:** O(nlogn)
- **Space Complexity:** O(n)

---

## Notes

remeber the skipping duplicate code
also the basics of two pointer - based on the different conditions we change the way we dec or inc

---

## Alternative Solutions

<!-- If you have multiple approaches, document them here -->

---

## Related Problems

<!-- Link to related problems if applicable -->
